...
func (data *TemplateData) GetEnvVars() ([]corev1.EnvVar, error) {
	...
	if cluster.Spec.Cloud.Kubevirt != nil {
		...
		if dc.Spec.Kubevirt != nil {
			allowPVCClone := dc.Spec.Kubevirt.Images.HTTP != nil && dc.Spec.Kubevirt.Images.HTTP.ImageCloning.Enable
			vars = append(vars, corev1.EnvVar{Name: "KUBEVIRT_ALLOW_PVC_CLONE", Value: strconv.FormatBool(allowPVCClone)})

			allowCustomImages := dc.Spec.Kubevirt.Images.EnableCustomImages
